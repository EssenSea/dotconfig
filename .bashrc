...
_atuin() {
    ...
    case "${cmd}" in
        atuin)
            opts="-h -V --help --version history import stats search sync login logout register key status account kv store dotfiles init info doctor daemon default-config uuid contributors gen-completions help"
        ...
        atuin__gen__completions)
            ...
            ;;
        atuin__help)
            opts="history import stats search sync login logout register key status account kv store dotfiles init info doctor daemon default-config uuid contributors gen-completions help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
